...
const GENLAYER_RPC = "https://studio.genlayer.com/api/rpc";
...
export default function Home() {
    ...
    // ── GenLayer RPC call ───────────────────────────────────────────
    const genLayerCall = async (method: string, params: unknown[]) => {
    ...
    const deployEscrow = async () => {
        ...
        try {
            ...
            // Send deployment transaction via GenLayer RPC
            const result = await genLayerCall("gen_deployContract", [
                wallet,
                {
            ...
            setTxHash(result);
            setTxStatus("success");
            setTxMsg(`Contract deployed! TX: ${result}`);
        } catch (e: unknown) {
            setTxStatus("error");
    ...
    const fetchState = async () => {
        ...
        try {
            const result = await genLayerCall("gen_call", [
                { to: contractAddr, data: { method: "get_status", args: [] } },
            ]);
            // Parse state from result
    ...
    const markComplete = async () => {
        if (!provider || !wallet) { alert("Connect wallet first."); return; }
        setTxStatus("pending");
        setTxMsg("Sending mark_complete transaction...");
        try {
            const result = await genLayerCall("gen_sendTransaction", [
                {
                    from: wallet,
                    to: contractAddr,
                    data: { method: "mark_complete", args: [] },
                },
            ]);
            setTxHash(result);
            setTxStatus("success");
            ...
            const result = await genLayerCall("gen_sendTransaction", [
                {
                    from: wallet,
                    to: contractAddr,
                    data: { method: "release_payment", args: [] },
                },
            ]);
            setTxHash(result);
            setTxStatus("success");
        ...
        <div className="min-h-screen grid-bg">
            ...
            <header className="border-b border-[#1a1a2e] backdrop-blur-sm sticky top-0 z-50 bg-[#0a0a0f]/80">
                <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <button
                        ...
                        className={`font-mono text-sm px-4 py-2 rounded-lg transition-all ${wallet
            ...
            <main className="max-w-5xl mx-auto px-6 py-12">
                ...
                <div className="text-center mb-16 animate-fade-up-1">
                    ...
                    <p className="text-[#606070] max-w-xl mx-auto text-lg leading-relaxed">
                        Smart contracts powered by GenLayer AI verify work completion and
                        release payments — no middlemen, no disputes.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-12 animate-fade-up-2">
                    {[
                        ...
                        <div key={stat.label} className="card rounded-xl p-6 text-center glow-border">
                            <div className="font-mono text-2xl font-bold text-[#00ff88] mb-1">
                                {stat.value}
                            </div>
                            <div className="text-xs text-[#505060] font-mono tracking-wider">
                    ...
                    {tab === "create" && (
                        <div className="grid sm:grid-cols-2 gap-8">
                            <div className="space-y-5">
                                <div>
                                    <label className="block font-mono text-xs text-[#505060] tracking-wider mb-2">
                                        JOB DESCRIPTION
                                    </label>
                                    <textarea
                                ...
                                <div>
                                    ...
                                    <input
                                        className="input-field w-full rounded-lg px-4 py-3 text-sm font-mono"
                                        placeholder="0x..."
                                        value={freelancer}
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input
                                            className="input-field w-full rounded-lg px-4 py-3 text-sm font-mono pr-16"
                                            placeholder="100"
                                            type="number"
                                            value={amount}
                                            onChange={(e) => setAmount(e.target.value)}
                                        />
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-mono text-[#404060]">
                                            GEN
                                        </span>
                                    </div>
                                </div>
                                ...
                                {!wallet && (
                                    <p className="text-xs text-[#505060] font-mono text-center">
                            ...
                            <div className="space-y-4">
                                ...
                                {[
                                    ...
                                    <div key={item.step} className="flex gap-4 p-4 card rounded-xl">
                                        <span className="font-mono text-[#00ff88] text-lg font-bold shrink-0">
                                            {item.step}
                                        </span>
                                        <div>
                    ...
                    {tab === "manage" && (
                        <div className="space-y-6">
                            <div className="flex gap-3">
                                <input
                                    className="input-field flex-1 rounded-lg px-4 py-3 text-sm font-mono"
                                    placeholder="Contract address 0x..."
                                    value={contractAddr}
                                    onChange={(e) => setContractAddr(e.target.value)}
                                />
                                <button onClick={fetchState} className="btn-secondary px-5 rounded-lg font-mono text-sm">
                                    LOAD
                                </button>
                            </div>
                            ...
                            {escrowState && (
                                <div className="card rounded-xl p-6 space-y-5 glow-border">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <span className={`text-xs font-mono px-2 py-1 rounded-full ${escrowState.paid ? "badge-paid" :
                                            escrowState.completed ? "badge-complete" : "badge-active"
                                            }`}>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-sm">
                                        <div>
                                            <p className="text-[#505060] text-xs font-mono mb-1">CLIENT</p>
                                            <p className="font-mono text-[#00ff88]">{short(escrowState.client)}</p>
                                        </div>
                                        <div>
                                            <p className="text-[#505060] text-xs font-mono mb-1">FREELANCER</p>
                                            <p className="font-mono text-[#00ff88]">{short(escrowState.freelancer)}</p>
                                        </div>
                                        <div>
                                            <p className="text-[#505060] text-xs font-mono mb-1">AMOUNT</p>
                                            <p className="font-mono text-white">{escrowState.amount} GEN</p>
                                        </div>
                                        <div>
                                            <p className="text-[#505060] text-xs font-mono mb-1">WORK STATUS</p>
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        <button
                                            ...
                                            disabled={escrowState.completed || !wallet}
                                            className="btn-secondary flex-1 py-2.5 rounded-lg font-mono text-sm"
                                        >
                                            ✓ Mark Complete
                                        </button>
                                        <button
                                            onClick={releasePayment}
                                            disabled={!escrowState.completed || escrowState.paid || !wallet}
                                            className="btn-primary flex-1 py-2.5 rounded-lg font-mono text-sm"
                                        >
                                            ⊕ Release Payment
                                        </button>
                                    </div>
                    ...
                    {tab === "about" && (
                        <div className="grid sm:grid-cols-2 gap-6">
                            {[
                                {
                                    icon: "⬡",
                                    title: "GenLayer Smart Contracts",
                                    desc: "Built on GenLayer's Intelligent Contract platform — Python contracts that can reason, access the internet, and make AI-powered decisions.",
                                },
                                {
                                    icon: "⊕",
                                    title: "Trustless Escrow",
                                    desc: "Funds are locked in the contract until work is verified complete. No human intermediary can steal or delay your payment.",
                                },
                                {
                                    icon: "◈",
                                    title: "AI Verification",
                                    desc: "Multiple AI validators reach consensus on whether work meets requirements — objective, automated, incorruptible.",
                                },
                                {
                                    icon: "⊞",
                                    title: "Open Source",
                                    desc: "All contract code is public and auditable. You can verify exactly how payment release logic works before locking funds.",
                                },
                            ].map((item) => (
                ...
                {/* ── TX Status Toast ────────────────────────────────────── */}
                {txStatus !== "idle" && (
                    <div className={`fixed bottom-6 right-6 max-w-sm p-4 rounded-xl font-mono text-sm border animate-fade-up z-50 ${txStatus === "pending" ? "bg-[#0f0f1a] border-[#ffa50040] text-[#ffa500]" :
                        txStatus === "success" ? "bg-[#0f0f1a] border-[#00ff8840] text-[#00ff88]" :
                            "bg-[#0f0f1a] border-[#ff444440] text-[#ff4444]"
                        }`}>
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-xs opacity-60 mb-1">
                                    {txStatus === "pending" ? "TRANSACTION PENDING" : txStatus === "success" ? "SUCCESS" : "ERROR"}
                                </p>
                                <p className="text-xs leading-relaxed break-all">{txMsg}</p>
                                {txHash && (
                                    <p className="text-xs opacity-50 mt-1 break-all">{short(txHash)}</p>
                                )}
                            </div>
                            <button
                                onClick={() => { setTxStatus("idle"); setTxMsg(""); }}
                                className="text-xs opacity-40 hover:opacity-80 ml-auto shrink-0"
                            >
                                ✕
                            </button>
                        </div>
            ...
            <footer className="border-t border-[#1a1a2e] mt-20 py-8">
                <div className="max-w-5xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <span className="font-mono text-xs text-[#303040]">
                        CHAINESCROW © 2026 — BUILT ON GENLAYER
                    </span>
                    <div className="flex gap-6">
                        {["Docs", "GitHub", "Discord"].map((l) => (
                            <a key={l} href="#" className="font-mono text-xs text-[#303040] hover:text-[#00ff88] transition-colors">
                                {l}
                            </a>
                        ))}
                    </div>
