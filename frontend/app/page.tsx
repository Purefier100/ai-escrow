...
export default function Home() {
    ...
    const postJobListing = async () => {
        ...
        try {
            ...
            // Deploy contract with zero address as freelancer (open position)
            const hash = await client.deployContract({
                code: getContractCode(),
            ...
            for (let i = 0; i < 20; i++) {
                ...
                const topic2 = receipt?.result?.logs?.[0]?.topics?.[2];
                if (topic2) { contractAddress = "0x" + topic2.slice(26); break; }
                setTxMsg(`Waiting... (${i + 1}/20)`);
            }

            // Save job listing with contract address
            ...
            setTxStatus("success");
            setTxMsg(`Job posted! Contract: ${contractAddress}`);
                                                ...
                                                <button
                                                    onClick={() => {
                                                        if (job.contractAddr) {
                                                            ...
                                                            alert("This job doesn't have a contract yet.");
                                                        }
                                                    }}
                                                </button>
                                            )}
...
class Escrow(gl.Contract):
    ...
    def mark_complete(self, work_url: str):
        # Any address can submit work (open application)
