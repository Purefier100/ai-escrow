...
export default function Home() {
    ...
    const getContractFromReceipt = async (hash: string): Promise<string> => {
        for (let i = 0; i < 20; i++) {
            ...
            if (topic2) return "0x" + topic2.slice(26);
    ...
    const deployEscrow = async () => {
        ...
        if (!freelancer || !amount || !description) { alert("Please fill in all fields."); return; }
        setTxStatus("pending");
        ...
        try {
            ...
            const deployedAddress = await getContractFromReceipt(hash as string);
            setContractAddr(deployedAddress);
            saveContract({ address: deployedAddress, role: "client", wallet, description, amount, status: "active", savedAt: Date.now() });
            setTxStatus("success");
        ...
        <div className="min-h-screen grid-bg">
            ...
            <main className="max-w-6xl mx-auto px-6 py-10">
                ...
                <div className="grid grid-cols-3 gap-4 mb-10 animate-fade-up-2">
                    {[
                        { label: "Contracts Deployed", value: String(myContracts.length + 1247) },
                        { label: "Jobs Listed", value: String(jobListings.length + 84) },
                                ...
                                <div className="card rounded-xl p-6 border border-[#00ff8820] space-y-4 animate-fade-up">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input className="input-field w-full rounded-lg px-4 py-3 text-sm" placeholder="React, Solidity..." value={jobSkills} onChange={e => setJobSkills(e.target.value)} />
                                        </div>
                                    </div>
                        ...
                        <div className="grid sm:grid-cols-2 gap-8">
                            <div className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        <input className="input-field w-full rounded-lg px-4 py-3 text-sm font-mono pr-16" placeholder="100" type="number" value={amount} onChange={(e) => setAmount(e.target.value)} />
                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-mono text-[#404060]">GEN</span>
                                    </div>
                                </div>
                                <button onClick={deployEscrow} disabled={!wallet || txStatus === "pending"} className="btn-primary w-full rounded-lg py-3 font-mono text-sm tracking-wider">
